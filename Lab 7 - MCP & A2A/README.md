...
1. After seeing the MCP provide the tools dynamically, it became clear to me that these automations are highly superior. Hard-coding anything is just asking for trouble because those tools are going to be updated and break. MCP allows us to dynamically build in tools to our system. They add a lot of value to the overall system. On the flip side, they do require an additional bit of infrastructure and error handling, such as handling what to do if we can't retreive the tools, or knowing how the tools fail, etc.

2. Knowing what to include in the context window was a bit trickier. Obviously, the more you pass in makes it harder for the model to give attention to everything. But knowing what to include can be tricky since the tools change dynamically. This is why having something like the MCP is so important, because it can help us determine what to include in the context window. We can use the MCP to determine what tools are available and then include those in the context window. This way, we can ensure that we are including the most relevant information for the model to make a decision.

3. We could let the LLM decide the order by giving it the tools and letting it decide which one to use first. This would allow the model to make a decision based on the tools available and the context of the conversation. However, this could also lead to suboptimal decisions if the model doesn't have enough information to make a good decision. Alternatively, we could have a predefined order for the tools, but this could also lead to suboptimal decisions if the order is not optimal for the given situation. Ultimately, I think it would be best to let the model decide the order based on the context of the conversation and the tools available.

4. I would want to have an MCP include everything that we could possibly need to make a decision. This would include the tools available, the context of the conversation, and any relevant information that could help the model make a decision. I would also want to include any relevant information about the user, such as their preferences or past interactions, as this could help the model make a more personalized decision. Additionally, I would want to include any relevant information about the task at hand, such as the goals or constraints, as this could help the model make a more informed decision.
